...
//    private Scanner s;
    ...
    static void prod_input(Scanner s){
        ...
        // TODO:
        System.out.println("[System] 제품 등록을 원하는 제품명을 입력하시오: ");
    ...
    static void prod_remove(Scanner s){
        ...
        for (int i = 0; i < products.length; ++i){
            if (remove.equals(products[i])){  // 입력한 제품명과 일치하는 제품을 찾으면
                foundIdx = i;  // 인덱스 저장
                break;  // 반복 중단
            }
        }
    ...
    public static void prod_amount_add(Scanner s) {
        ...
        String cntValidate = "";
            ...
            // TODO: + 유효성 검사하기 아래 코드에서.
            cntValidate = s.nextLine();
            for (int i = 0; i < cntValidate.length(); i++) {
                if (!Character.isDigit(cntValidate.charAt(i))) {
        ...
        int cnt = Integer.parseInt(cntValidate); // 수량 입력 받기
    ...
    public static void prod_amount_decrease(Scanner s) {
        // 제품 수량 감축 로직을 구현합니다. 입력받은 수량만큼 해당 제품의 수량을 감소시킵니다. + 101개 못뺌
        System.out.println("제품의 출고를 진행합니다.");
        prod_search();


        System.out.print("[System] 출고를 진행할 제품명을 입력하세요 : ");
        String output = s.nextLine();  // 제품명 입력 받기
        int foundIdx = -1;  // 찾은 인덱스 초기화
        for (int i = 0; i < products.length; ++i){
...
//Character.isDiget(char c) = true
        int cnt = Integer.parseInt(cntValidate);  // 수량 입력 받기
        if (cnt > counts[foundIdx]) {
            System.out.println("남은 수량보다 더 많이 출고할 수 었습니다.");
